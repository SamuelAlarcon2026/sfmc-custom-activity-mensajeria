...
function getAllowedTestDomains() {
  return String(process.env.TEST_SEND_ALLOWED_DOMAINS || '')
    .split(',')
    .map((item) => item.trim().toLowerCase())
    .filter(Boolean);
}

function assertAllowedTestRecipient(email) {
  const domains = getAllowedTestDomains();
  if (!domains.length) return;

  const domain = String(email || '').split('@').pop().toLowerCase();
  if (!domains.includes(domain)) {
    throw new AppError(
      `Dominio no permitido para envío de test: ${domain}`,
      403,
      {
